...
export function ReportDownload({ reportId }: ReportDownloadProps) {
  ...
  const handlePdfDownload = async () => {
    ...
    try {
      ...
      const html = `
        ...
        <head>
          <meta charset="utf-8" />
          <title>CIETO Report — ${rec?.factory_name ?? factoryName}</title>
          <style>
        ...
        <body>
          <div class="brand"><div class="brand-dot"></div><span style="font-weight:900;font-size:18px;letter-spacing:2px">CIETO</span><span style="color:#6b7280;font-size:12px">ENERGY PLATFORM</span></div>
          ...
          <div class="disclaimer">Generated by CIETO Energy Platform &bull; Prototype v1.0 &bull; All figures are estimates based on MSME-provided inputs. This report does not constitute financial or legal advice.</div>
        </body>
        </html>
      `
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      const rows = [
        ["CIETO Energy Transition Report"],
