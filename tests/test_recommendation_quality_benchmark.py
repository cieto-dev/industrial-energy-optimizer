...
def build_benchmark_cases() -> list[BenchmarkScenario]:
    ...
    cases.append(
        BenchmarkScenario(
            ...
            current_fuel="Petcoke + coal",
            baseline_annual_opex_inr=50_000_000,
            process_temperature_c=1400,
            biomass_available=True,
            renewable_electricity_available=True,
            grid_capacity_kw=4500,
            required_electric_capacity_kw=3200,
            budget_inr=30_000_000,
...
def test_co2_reduction_within_research_band(case: BenchmarkScenario):
    ...
    selected_candidate = next(
        candidate
        for candidate in case.candidate_scenarios
        if candidate.scenario_id == output["selected"].scenario_id
    )

    selected_co2 = selected_candidate.co2_reduction_pct
...
def test_cement_is_not_treated_as_fully_decarbonizable_by_fuel_switch():
    ...
    selected_candidate = next(
        c
        for c in cement.candidate_scenarios
        if c.scenario_id == output["selected"].scenario_id
    )

    assert selected_candidate.co2_reduction_pct < 50.0
...
def test_benchmark_summary_has_nine_results():
    ...
    for result in results:
        ...
        selected_candidate = next(
            candidate
            for candidate in result["case"].candidate_scenarios
            if candidate.scenario_id == result["selected"].scenario_id
        )
        assert selected_candidate.co2_reduction_pct is not None
